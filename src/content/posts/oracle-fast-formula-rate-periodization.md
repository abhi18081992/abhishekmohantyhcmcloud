...
description: "Oracle Fast Formula: Rate Periodization — When the Standard Rate Engine Isn't Enough"
pubDate: 2026-03-31
...
March 2026 · 20 min read · Oracle HCM Cloud
...
Oracle ACE Apprentice · AIOUG Member · HCM Cloud Consultant
...
#### Cap Fires
Prorated ($750) ≤ YTD ($800)ANN = 0 | DFND = 0 | CMCD = 0
...
Step 1 — Proration
Proration month = 07Proration factor = .5


Step 2 — YTD Loop
Hire Date = 2025/01/15Plan Year Start = 2025/01/01First comp date = 2025/01/01
...
IN`BEN_IV_CONVERT_FROM`TextDEFINED, ANNUAL, or CMCD
IN`BEN_IV_CONVERT_FROM_VAL`NumberThe raw rate amount
OUT`DFND_VAL`NumberMandatory
OUT`ANN_VAL`NumberMandatory
OUT`CMCD_VAL`NumberMandatory
...
WHILE ((l_comp_date AND (l_count 13)) LOOP
(
...
IF l_prorated_entitlement THEN
(
  ANN_VAL  = 0
  DFND_VAL = 0
  CMCD_VAL = 0
...
→
CHANGE_CONTEXTSShift to that election
→
TO_CHAR(CVG_STRT_DT, 'MM')Extract month number
→
(13 − month) / 12Proration factor
...
The `WHILE` condition has two guards: `l_comp_date EFFECTIVE_DATE = l_comp_date

Tells the DBI: "read the entry as of this month"


ACTY_BASE_RT_ID = l_acty_id

Tells the DBI: "read the entry for this specific rate"


**How does the formula know to read ER and not EE?**
...
IF entitlement ($750) ≤ YTD ($800)


ANN_VAL  = 0**DFND_VAL = 0
CMCD_VAL = 0

Already overpaid. Zero everything. No further deposits.


ELSE entitlement ($750) > YTD ($200)

l_balance = 750 − 200 = 550
ANN_VAL  = 550
DFND_VAL = 550 / 12 = 45.83
CMCD_VAL = 550 / 24 = 22.92

Balance remaining. Split three ways.
...
3. ` If the entitlement exactly equals the YTD total, the cap fires. The employer owes nothing more. Using `
...
Oracle ACE Apprentice · AIOUG Member · HCM Cloud Consultant & Technical Lead — Fast Formulas, Absence Management, Time & Labor, Core HR, Redwood, HDL, OTBI.
